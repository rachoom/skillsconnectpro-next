...
//// 🟢 COMPONENT: SMART SCROLL INDICATOR
const ScrollIndicator: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Calculates if the user is within 100px of the bottom of the page
  ...
  return (
   <div 
      className={`fixed z-[150] pointer-events-none transition-all duration-700 right-4 bottom-28 md:right-8 md:bottom-32 ${
        isVisible && !isMobile ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
      }`}
    >
      <div className="flex flex-col items-center animate-bounce">
        <div className="bg-black/60 backdrop-blur-md border border-brand-yellow/40 text-brand-yellow p-2 md:p-3 rounded-full shadow-[0_0_20px_rgba(250,204,21,0.2)]">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={3} stroke="currentColor" className="w-3 h-3 md:w-5 md:h-5">
...
const App: React.FC<AppProps> = ({ deepLinkProfileId }) => { // <--- Receive prop
 ...
 // 🚀 Deep Link Listener
  useEffect(() => {
    // If we have a profileId and data is ready, auto-open it
    if (deepLinkProfileId && artisans.length > 0) {
        console.log("🚀 Deep-linking to Profile ID:", deepLinkProfileId);
        const matchedArtisan = artisans.find(a => a.id.toString() === deepLinkProfileId);
        if (matchedArtisan) {
            setSelectedArtisan(matchedArtisan);
            setAppState(AppState.PROFILE); // Open the actual profile view!
  ...
  const findClosestLocation = (input: string): string | null => {
   ...
   // Update findClosestLocation aliases
const locationAliases: Record<string, string> = {
  // Common misspellings / shorthands
  'tsakani': 'Tsakane',
  'kwate': 'KwaThema',
  'kwa thema': 'KwaThema',
  'kwa-thema': 'KwaThema',
  ...
  const goToListingsDirectly = () => {
    setAppState(AppState.HOME); // This loads listings section
  };

  const handleFindService = () => {
    goToListingsDirectly(); // Go to Listings page
...
const { error } = await supabase.from('artisan_applications').insert([
    { 
        ...
        proof_of_work: uploadedImageUrls
    }
]);
     ...
     {isMobileMenuOpen && (
          <div className={`md:hidden absolute top-full left-0 right-0 border-b shadow-2xl animate-fade-in z-40 ${isDarkMode ? 'bg-[#150f0a] border-white/10' : 'bg-white border-gray-200'}`}>
            <div className="flex flex-col p-6 space-y-6 text-center text-sm font-black uppercase tracking-widest">
              ...
              {/* 👇 HERE IS YOUR NEW MOBILE BLOG LINK 👇 */}
              <Link href="/blog" onClick={() => setIsMobileMenuOpen(false)} className={`py-3 ${isDarkMode ? 'text-brand-yellow hover:text-white' : 'text-brand-yellow hover:text-black'}`}>Blog</Link>
            </div>
          </div>
        )}
      </nav>
...
<div className="absolute inset-0 z-0 bg-[#0c0906]">
  ...
  <div className="absolute inset-0 bg-gradient-to-b from-[#150f0a]/95 via-[#150f0a]/70 to-[#150f0a] z-10"></div>
              ...
              {/* Centered Content Column */}
                <div className="relative z-10 flex flex-col items-center text-center w-full max-w-4xl mx-auto gap-4 md:gap-6">
                  ...
                  <div className="bg-brand-yellow/10 border border-brand-yellow/30 text-brand-yellow px-3 py-1.5 md:px-5 md:py-2 rounded-full text-[8px] md:text-xs font-black uppercase tracking-[0.25em] shadow-[0_0_20px_rgba(250,204,21,0.15)] mt-4 md:mt-0">
                    Far East Rand Specialist Network
                  </div>

                  <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black tracking-tighter text-white leading-[1.1] md:leading-[1.05]">
                    Find Verified{' '}
                  ...
                  <p className={`text-sm sm:text-base md:text-xl font-medium max-w-2xl leading-relaxed px-4 ${isDarkMode ? 'text-gray-400 md:text-gray-300' : 'text-gray-600 md:text-gray-700'}`}>
                    Discover top-tier professionals in <span className="text-brand-yellow font-bold">Tsakane, Vosloorus, Tembisa</span> and more.
                  </p>

                  {/* ── THE GOOGLE-STYLE SEARCH BAR ── */}
                  {/* 🛠️ FIX: Set wrapper px-2 on mobile instead of px-0 to give breathing room. */}
                  <div className="w-full max-w-3xl mt-2 md:mt-4 relative group px-2 md:px-0">
                    
                    {/* 🛠️ FIX: Made absolute positioned blur background responsive (`inset-0 blur-md` on mobile, `-inset-1.5 blur-xl` on desktop) so it doesn't cause horizontal scrollbars or muddy up the UI. */}
                    <div className="absolute inset-0 md:-inset-1.5 bg-gradient-to-r from-brand-yellow to-yellow-500 rounded-[3rem] blur-md md:blur-xl opacity-50 md:opacity-25 group-hover:opacity-80 transition-opacity duration-500 pointer-events-none"></div>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 md:gap-4 truncate">
                        <SearchIcon className="w-4 h-4 md:w-6 md:h-6 text-brand-yellow shrink-0" />
                        <span className="text-[12px] md:text-xl font-bold text-zinc-500 text-left truncate">
                          <span className="hidden md:inline">Search: </span>
                          <span className="text-zinc-500 md:text-white">Plumber, Electrician...</span>
                        </span>
                      </div>
                  ...
                  {/* 🛠️ FIX: Adjusted margins (`mt-6`) to stop the chips from crowding the search bar */}
                  <div className="flex flex-wrap justify-center gap-1.5 md:gap-2 mt-6 md:mt-4 px-2">
            ...
            {/* --- DIRECTORY SECTION --- */}
              <section className="w-full mt-20 max-w-7xl mx-auto px-4 md:px-8">
                
                {/* 1. NEW POPULAR TRADES HEADING */}
                <div className="flex flex-col items-center mb-12 reveal">
                  <h3 className="text-[10px] font-black uppercase tracking-[0.5em] text-brand-yellow mb-4">Directory</h3>
                  <h2 className={`text-4xl md:text-5xl font-black tracking-tighter text-center ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Popular Trades</h2>
                  <div className="h-1 w-20 bg-brand-yellow mt-6"></div>
                </div>

                {/* 2. NEW POPULAR TRADES CARDS */}
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6 mb-24 perspective-container" onMouseEnter={() => setRovingIndex(null)}>
                  {QUICK_CATEGORIES.map((cat, index) => {
                    ...
                    <button 
                      ...
                      >
                        ...
                        <div className={`absolute inset-0 bg-gradient-to-t transition-opacity duration-500 ${isDarkMode ? 'from-black/95 via-black/40 to-transparent' : 'from-black/90 via-black/30 to-transparent'} ${isRoving ? 'opacity-100' : 'opacity-90 group-hover:opacity-100'}`} />
                        
                        <div className="absolute inset-0 p-6 flex flex-col justify-end z-30 text-flat">
                          {/* Yellow Label Pill */}
                          <div className={`bg-brand-yellow/90 backdrop-blur-sm text-black w-fit px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest mb-4 transition-all duration-500 ${isRoving ? 'scale-105' : ''}`}>High Demand</div>
                          
                          {/* Main Title */}
                          <h3 className={`text-xl md:text-2xl font-black mb-6 leading-tight text-white transition-colors ${isRoving ? 'text-brand-yellow' : 'group-hover:text-brand-yellow'}`}>
                            {cat.label}
                          </h3>
                ...
                <div className="reveal perspective-container">
                  {/* 3. NEW ADDITIONAL SERVICES HEADING */}
                  <div className="flex flex-col items-center mb-12 mt-12">
                    <h3 className="text-[10px] font-black uppercase tracking-[0.5em] text-brand-yellow mb-4">More Options</h3>
                    <h2 className={`text-4xl md:text-5xl font-black tracking-tighter text-center ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Additional Services</h2>
                    <div className="h-1 w-20 bg-brand-yellow mt-6"></div>
                  </div>
                      ...
                      >
                        ...
                        <div className="absolute inset-0 p-8 flex flex-col justify-end z-30 text-flat">
                          ...
                          <div className="flex items-center gap-3 mb-4">
                            <div className="w-10 h-10 rounded-full bg-brand-yellow text-black flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform *:w-5 *:h-5">
                              {service.icon} 
                            </div>
                            <div className="bg-brand-yellow/90 backdrop-blur-sm text-black w-fit px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest">Specialty</div>
                          </div>
                          
                          <h3 className="text-2xl font-black mb-6 leading-tight text-white group-hover:text-brand-yellow transition-colors">
                            {service.label}
                          </h3>
          ...
          {appState === AppState.PROFILE && selectedArtisan && (
            <div className={`min-h-screen pt-24 pb-12 px-6 relative animate-fade-in ${isDarkMode ? 'bg-[#150f0a] text-white' : 'bg-white text-gray-900'}`}>
              <button onClick={() => { setAppState(AppState.SEARCH_RESULTS); // Back to Search Results logic (Turn 13 Flow Fix)
                  window.scrollTo({ top: 0, behavior: 'auto' });
                 }} className="mb-8 flex items-center gap-2 text-gray-400 hover:text-brand-yellow transition-colors">
  ...
  <div className="animate-fade-in max-w-3xl mx-auto px-4 sm:px-6 py-24 w-full reveal relative min-h-screen flex flex-col justify-center">
    
    {/* 1. TECHY BACKGROUND & ORBS */}
    <div className="fixed inset-0 -z-30 bg-[#070503]"></div>
    <div className="fixed inset-0 -z-20 bg-tech-grid pointer-events-none"></div>
    <div className="fixed top-1/4 -left-20 w-[500px] h-[500px] bg-brand-yellow glowing-orb pointer-events-none"></div>
    <div className="fixed bottom-1/4 -right-20 w-[500px] h-[500px] bg-yellow-600 glowing-orb pointer-events-none" style={{ animationDelay: '-5s' }}></div>

    {/* 2. BACK BUTTON */}
    <button onClick={goHome} className="mb-8 w-fit text-xs font-black uppercase tracking-widest text-gray-400 hover:text-brand-yellow flex items-center transition-all group z-10 bg-black/40 backdrop-blur-md px-4 py-2 rounded-full border border-white/5">
      <span className="mr-3 transition-transform group-hover:-translate-x-1">←</span> Back Home
    </button>

    {/* 3. FROSTED GLASS FORM CONTAINER */}
    <div className="glass-panel rounded-[2.5rem] p-8 sm:p-12 md:p-16 relative overflow-hidden" id="registration-form">
      
      {/* Circuit Accent Line at top */}
      ...
      {!isSubmitted ? (
        <div className="relative z-10">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-5 mb-10">
            <div className="w-14 h-14 rounded-2xl bg-brand-yellow/10 flex items-center justify-center border border-brand-yellow/30 shadow-[0_0_20px_rgba(250,204,21,0.15)] shrink-0">
          ...
          <form onSubmit={handleRegSubmit} className="space-y-6 stagger-child">
            ...
            <div className="mt-6">
              ...
              <select value={regForm.institution} onChange={(e) => setRegForm({...regForm, institution: e.target.value})} 
                ...
                <option value="Sedibeng TVET" className="text-white bg-[#150f0a]">Sedibeng TVET</option>
                <option value="Private Technical College" className="text-white bg-[#150f0a]">Private Technical College</option>
                <option value="Other" className="text-white bg-[#150f0a]">Other</option>
              </select>
            </div>
            ...
            <div>
              ...
              <select required value={regForm.referralSource} onChange={(e) => setRegForm({...regForm, referralSource: e.target.value})} 
                ...
                <option value="Word of Mouth" className="text-white bg-[#150f0a]">Word of Mouth</option>
                <option value="Google Search" className="text-white bg-[#150f0a]">Google Search</option>
                <option value="Other" className="text-white bg-[#150f0a]">Other</option>
              </select>
            </div>
                ...
                <div className="grid grid-cols-4 gap-3 mt-4">
                  {imagePreviews.map((src, index) => (
                    <div key={index} className="relative aspect-square rounded-xl overflow-hidden border border-white/10 group shadow-lg">
                      ...
                      <button type="button" onClick={() => removeImage(index)} className="absolute top-1 right-1 bg-red-600/90 backdrop-blur-sm text-white rounded-full w-6 h-6 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">x</button>
                    </div>
                  ))}
                </div>
              )}
            ...
            <button type="submit" disabled={isSubmitting} className="w-full relative group overflow-hidden bg-brand-yellow text-brand-black font-black py-5 mt-4 rounded-2xl uppercase tracking-[0.2em] text-sm shadow-[0_0_30px_rgba(250,204,21,0.2)] hover:shadow-[0_0_40px_rgba(250,204,21,0.4)] transition-all transform active:scale-[0.98]">
              <div className="absolute inset-0 w-full h-full bg-white/30 translate-x-[-100%] group-hover:animate-[shimmer_1.5s_infinite] skew-x-12"></div>
              <span className="relative z-10">{isSubmitting ? "Processing..." : "Submit Application"}</span>
            </button>
          </form>
        </div>
      ) : (
        <div className="text-center py-16 animate-fade-in relative z-10">
          <div className="w-24 h-24 bg-brand-yellow/10 border border-brand-yellow/30 rounded-full flex items-center justify-center text-brand-yellow mx-auto mb-8 shadow-[0_0_30px_rgba(250,204,21,0.2)]">
            <VerifiedIcon />
          </div>
          <h2 className={`text-4xl md:text-5xl font-black mb-4 tracking-tighter ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Application Received!</h2>
          <p className="text-gray-400 mb-10 font-medium">Welcome to the network.</p>
          <button onClick={goHome} className="px-12 py-4 rounded-full font-black uppercase tracking-widest shadow-xl transition-all border bg-black/40 backdrop-blur-md border-white/10 hover:border-brand-yellow text-white hover:text-brand-yellow">
            Return to Home
          </button>
        </div>
      )}
      ...
      <footer className={`border-t pt-24 pb-12 ${isDarkMode ? 'bg-[#150f0a] border-white/5' : 'bg-gray-100 border-gray-200'}`}>
        ...
        <div className="mb-10 border-b border-white/5 pb-10">
           <h4 className="text-gray-400 text-xs font-bold uppercase tracking-widest mb-6">Popular Local Searches</h4>
           <div className="flex flex-wrap justify-center gap-x-6 gap-y-4 text-[10px] uppercase tracking-wider text-gray-500 font-medium">
               <a href="https://find.skillsconnectpro.co.za/plumber-springs.html" target="_blank" rel="noopener noreferrer" className="hover:text-brand-yellow transition-colors">Plumber in Springs</a>
               <span className="opacity-20">•</span>
               <a href="https://find.skillsconnectpro.co.za/electrician-boksburg.html" target="_blank" rel="noopener noreferrer" className="hover:text-brand-yellow transition-colors">Electrician in Boksburg</a>
               <span className="opacity-20">•</span>
               <a href="https://find.skillsconnectpro.co.za/builder-benoni.html" target="_blank" rel="noopener noreferrer" className="hover:text-brand-yellow transition-colors">Builder in Benoni</a>
               <span className="opacity-20">•</span>
               <a href="https://find.skillsconnectpro.co.za/painter-brakpan.html" target="_blank" rel="noopener noreferrer" className="hover:text-brand-yellow transition-colors">Painter in Brakpan</a>
               <span className="opacity-20">•</span>
               <a href="https://find.skillsconnectpro.co.za/general%20contractor-tsakane.html" target="_blank" rel="noopener noreferrer" className="hover:text-brand-yellow transition-colors">Contractors in Tsakane</a>
           </div>
        </div>
     ...
     {/* --- AI CAMERA ASSISTANT --- */}
      ...
      {/* --- SCROLL INDICATOR --- */}
      {appState !== AppState.WELCOME && <ScrollIndicator />}
