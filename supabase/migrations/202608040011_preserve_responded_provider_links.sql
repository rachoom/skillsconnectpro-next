...
begin
  ...
  select li.status into v_existing_status
  from public.lead_invitations li
  where li.project_id = p_project_id
    and li.provider_id = p_provider_id
  for update;

  if v_existing_status in ('accepted', 'declined') then
    raise exception 'This provider has already responded. Their secure invitation link cannot be regenerated.';
  end if;
...
is 'Creates or refreshes one provider invitation while routing is open and before that provider has responded.';
