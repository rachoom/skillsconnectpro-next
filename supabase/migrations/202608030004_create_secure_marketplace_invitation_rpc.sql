...
begin
  ...
  if v_project.status in ('completed', 'cancelled', 'unfulfilled') then
    raise exception 'Providers cannot be invited while the project is %.', v_project.status;
  end if;
...
is 'Creates or refreshes one secure provider invitation for a verified Skills Connect marketplace administrator and returns the raw response token once.';
